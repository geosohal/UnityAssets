...
public class VectorGrid : MonoBehaviour 
{
    ...
    // Turn this on to revert the grid colors to their original values after forces stop being applied
    public bool m_ColorRevertEnabled = false;

    // Time to wait (in seconds) before color revert system activates
    public float m_ColorRevertDelay = 10.0f;

    // How quickly the color revert occurs
    public float m_ColorRevertSpeed = 0.1f;
    ...
    // External accessory, can be used to query grid point positions
    public VectorGridPoint[,] GridPoints { get { return m_VectorGridPoints; } set { m_VectorGridPoints = value; } }
	...
	{		
		...
		for (int x = 0; x < m_GridWidth; x++)
		{
			for (int y = 0; y < m_GridHeight; y++)
			{
				if(m_VectorGridPoints[x, y] != null)
				{
					m_VectorGridPoints[x, y].UpdatePositionAndColor(m_ColorRevertEnabled, m_ColorRevertDelay, m_ColorRevertSpeed);
				}
			}
		}
	}
		...
		if(m_VectorGridPoints != null)
		{
			...
			for(int x = 0; x < m_GridWidth; x++)
			{
				for(int y = 0; y < m_GridHeight; y++)
				{
					...
					{
                        VectorGridPoint gridPoint = m_VectorGridPoints[x, y];
                        Vector3 forceVector = gridPoint.m_Position - localPosition;
                        float distanceFactor = 1.0f - (forceVector.magnitude * invRadius);
						Vector3 forceVectorNormalized = forceVector.normalized;

                        gridPoint.m_Acceleration += (forceVectorNormalized * distanceFactor * force) * gridPoint.m_InverseMass;
		...
		if(m_VectorGridPoints != null)
		{
			...
			for(int x = 0; x < m_GridWidth; x++)
			{
				for(int y = 0; y < m_GridHeight; y++)
				{
					...
					{
                        VectorGridPoint gridPoint = m_VectorGridPoints[x, y];
                        Vector3 offset = gridPoint.m_Position - localPosition;
                        float distanceFactor = 1.0f - (offset.magnitude * invRadius);
                        gridPoint.m_Acceleration += (force * distanceFactor) * gridPoint.m_InverseMass;
                        gridPoint.m_Damping *= 0.6f;
                        gridPoint.m_LastForceTime = Time.time;
